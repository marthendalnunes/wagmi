...
test('context', () => {
  ...
  expectTypeOf(sendRawTransaction.data).toEqualTypeOf<Hash | undefined>()
  expectTypeOf(
    sendRawTransaction.error,
  ).toEqualTypeOf<SendRawTransactionErrorType | null>()
  expectTypeOf(sendRawTransaction.variables).toMatchTypeOf<
    { chainId?: number | undefined } | undefined
  >()
  expectTypeOf(sendRawTransaction.context).toEqualTypeOf<
    typeof contextValue | undefined
  >()

  sendRawTransaction.mutate(
